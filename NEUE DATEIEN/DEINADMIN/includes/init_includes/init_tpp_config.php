...
if (empty($_SESSION['admin_id'])) {
  ...
  $tpp_menu_text = 'Set Tabbed Products Pro options';
    ...
    $menu_items_tpp = array(
            array('TPP_VERSION','1.6.0',1001,false),
    ...
    if($original_config->RecordCount())
    {
        ...
        $sql = "INSERT INTO ".TABLE_CONFIGURATION_GROUP." (configuration_group_title, configuration_group_description, sort_order, language_id, visible) VALUES ('".$tpp_menu_title."', '".$tpp_menu_text."', ".$sort.", '43', '1')";
        $db->Execute($sql);
	...
	{
		$messageStack->add('Die nun nicht mehr benötigte Installationsdatei '.DIR_FS_ADMIN.'includes/auto_loaders/config.tpp.php wurde nicht automatisch gelöscht. Damit Tabbed Products Pro funktionieren kann, löschen Sie diese Datei nun bitte manuell.','error');
	};
    }
